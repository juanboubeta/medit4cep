...
public class ComplexEventTypeNameEditPart extends CompartmentEditPart implements ITextAwareEditPart {
	 ...
	 * of a ComplexEvent node, when the editor is loaded.
	 * @generated NOT
	 */
	protected Image getLabelIcon() {
		...
		if (eObject instanceof ComplexEvent) {
			imagePath = ((ComplexEvent) eObject).getImagePath();

			// If the path is wrong, the default attribute image will be shown.
			if (imagePath == null || imagePath.equals("") || !(new File(imagePath).exists()))
				return EventpatternElementTypes.getImage(parserElement.eClass());
	...
	public IParser getParser() {
		if (parser == null) {
			parser = EventpatternParserProvider.getParser(EventpatternElementTypes.ComplexEvent_2038,
					getParserElement(), EventpatternVisualIDRegistry
							.getType(eventpattern.diagram.edit.parts.ComplexEventTypeNameEditPart.VISUAL_ID));
		}
		return parser;
	}
