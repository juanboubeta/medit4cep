...
public class ComplexEventEditPart extends ShapeNodeEditPart {
	...
	protected void createDefaultEditPolicies() {
		...
		installEditPolicy(EditPolicyRoles.SEMANTIC_ROLE,
				new ComplexEventItemSemanticEditPolicy());
		...
		if (childEditPart instanceof ComplexEventTypeNameEditPart) {
			((ComplexEventTypeNameEditPart) childEditPart)
					.setLabel(getPrimaryShape()
							.getFigureComplexEventLabelFigure());
			return true;
		}
		if (childEditPart instanceof ComplexEventComplexEventComplexEventPropertiesCompartmentEditPart) {
			IFigure pane = getPrimaryShape()
					.getComplexEventComplexEventPropertiesCompartmentFigure();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane.add(((ComplexEventComplexEventComplexEventPropertiesCompartmentEditPart) childEditPart)
					.getFigure());
			return true;
		}
		return false;
	...
	protected boolean removeFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof ComplexEventComplexEventComplexEventPropertiesCompartmentEditPart) {
			IFigure pane = getPrimaryShape()
					.getComplexEventComplexEventPropertiesCompartmentFigure();
			pane.remove(((ComplexEventComplexEventComplexEventPropertiesCompartmentEditPart) childEditPart)
					.getFigure());
			return true;
		}
		return false;
	...
	public List<IElementType> getMARelTypesOnSourceAndTarget(
		...
		if (targetEditPart instanceof eventpattern.diagram.edit.parts.ComplexEventEditPart) {
			types.add(EventpatternElementTypes.Link_4001);
		}
		...
		if (event.getNotifier() == getModel()
			...
			if (event.getFeature() instanceof EAttribute) {
				...
				if (eAttribute.getName().equalsIgnoreCase("imagePath")) {
					ComplexEventFigure figure = (ComplexEventFigure) this
							.getPrimaryShape();
					...
					if (elementName != null && !elementName.equals("")
						...
						figure.getFigureComplexEventLabelFigure().setIcon(
								scaledImage);

						EventpatternDiagramEditor editor = (EventpatternDiagramEditor) PlatformUI
								.getWorkbench().getActiveWorkbenchWindow()
								.getActivePage().getActiveEditor();
						// editor.getTitle() returns xxx.pattern_diagram
						String activePatternName = editor.getTitle().replace(
								".pattern_diagram", "");

						EventPatternsStatus.getComplexEventChanges().put(
								activePatternName, true);
					}
	...
	public class ComplexEventFigure extends RectangleFigure {
		...
		private void createContents() {
			...
			fFigureComplexEventLabelFigure.setMaximumSize(new Dimension(
			...
			fComplexEventComplexEventPropertiesCompartmentFigure
					.setOutline(false);
