...
public class EventpatternMarkerNavigationProvider extends AbstractModelMarkerNavigationProvider {
	...
	protected void doGotoMarker(IMarker marker) {
		...
		if (targetEditPart != null) {
			EventpatternDiagramEditorUtil.selectElementsInDiagram(editor,
	...
	public static void deleteMarkers(IResource resource) {
		try {
			...
			EventpatternDiagramEditorPlugin.getInstance().logError("Failed to delete validation markers", e); //$NON-NLS-1$
		}
	}
	...
	public static IMarker addMarker(IFile file, String elementId, String location, String message, int statusSeverity) {
		...
		try {
			...
			EventpatternDiagramEditorPlugin.getInstance().logError("Failed to create validation marker", e); //$NON-NLS-1$
		}
		return marker;
	}
}
