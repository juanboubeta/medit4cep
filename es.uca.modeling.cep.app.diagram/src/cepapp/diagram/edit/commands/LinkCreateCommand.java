...
public class LinkCreateCommand extends EditElementCommand {
	...
	* @generated
	*/
	private final CEPApp container;
	...
	*/
	public boolean canExecute() {
		...
		if (source != null && false == source instanceof Origin) {
			return false;
		}
		...
		return CepappBaseItemSemanticEditPolicy.getLinkConstraints().canCreateLink_4001(getContainer(), getSource(),
				getTarget());
	}
	...
	*/
	protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info) throws ExecutionException {
		...
		Link newElement = CepappFactory.eINSTANCE.createLink();
	...
	* @generated
	*/
	private static CEPApp deduceContainer(EObject source, EObject target) {
		// Find container element for the new link.
		// Climb up by containment hierarchy starting from the source
		// and return the first element that is instance of the container class.
		for (EObject element = source; element != null; element = element.eContainer()) {
			if (element instanceof CEPApp) {
				return (CEPApp) element;
			}
		}
		return null;
	}
