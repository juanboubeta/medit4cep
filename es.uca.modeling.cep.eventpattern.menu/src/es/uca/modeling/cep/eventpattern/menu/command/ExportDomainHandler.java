...
public class ExportDomainHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
        ...
        if (ieditorpart == null || !ieditorpart.getTitle().endsWith("domain_diagram")) {
        	MessageDialog.openError(shell, "Export CEP Domain", "The domain must be open.");
        	return null; 
        }
		...
		if (problems.length > 0) {
			MessageDialog.openError(shell, "Export CEP Domain", "There are some problems that must be solved before exporting this domain.");
        	return null; 
		}
	    ...
	    dirDialog.setText("Choose a folder where to export the domain.");
        ...
        try {
            ...
            while ((inputLine = reader.readLine()) != null) {
                ...
                if (!hrefNotFound && inputLine.contains("<element xmi:type=\"domain:CEPDomain\" href=\"")) {
        ...
        if (((CEPDomain) modelResource.getContents().get(0)).getDomainName() != null && 
        		!((CEPDomain) modelResource.getContents().get(0)).getDomainName().trim().isEmpty()) {
        	domainName = ((CEPDomain) modelResource.getContents().get(0)).getDomainName().trim();
        }
        else {   // This has already been checked with EVL
        	MessageDialog.openError(shell, "Export CEP Domain", "'Domain Name' property cannot be empty.");
        	return null; 	       	
        }
		...
		for (Iterator iter = EcoreUtil.getAllContents(modelResource, true); iter.hasNext(); ) {
			...
			if (objectName.equals("EventPropertyImpl")) {
				...
				if (((EventProperty) eObject).getImagePath() != null && !((EventProperty) eObject).getImagePath().trim().isEmpty()) {
					imagePath = FileSystems.getDefault().getPath(((EventProperty) eObject).getImagePath().toString());
					imageFileName = imagePath.getFileName().toString(); 
					
					if (imageFileName != null) { 
						((EventProperty) eObject).setImagePath(imageFileName); 
					}
				}
			}
			else if (objectName.equals("EventImpl")) {
							
				if(((Event) eObject).getImagePath() != null && !((Event) eObject).getImagePath().trim().isEmpty()) {
					imagePath = FileSystems.getDefault().getPath(((Event) eObject).getImagePath().toString());
					imageFileName = imagePath.getFileName().toString(); 
								
					if(imageFileName != null) { 
						((Event) eObject).setImagePath(imageFileName); 
					}
				}
			}
		...
		URI modelFileUri = URI.createURI("file:///" + tempDirPath + domainName + ".domain");
		...
		diagramNewText = diagramOriginalText.replaceFirst("name=\".*.domain_diagram", 
				"name=\"" + domainName + ".domain_diagram");
				
		diagramNewText = diagramNewText.replaceAll("href=\".*.domain", "href=\"" + domainName + ".domain");

		File newfile = new File(tempDirPath + domainName + ".domain_diagram");
		FileWriter fw;
		...
		String zipPath = selectedDir + "/" + domainName + "_domain.zip";
