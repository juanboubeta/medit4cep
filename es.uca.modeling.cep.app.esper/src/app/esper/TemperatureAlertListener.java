...
public class TemperatureAlertListener implements UpdateListener {
	...
	private static Logger logger = Logger.getLogger(TemperatureAlertListener.class);
	private static ArrayList<String> lines = new ArrayList();
	
	// The detected complex events will be saved in the file located in the folder simulation-output.
	private static Path patternOutput = Paths.get("simulation-output/TemperatureAlert-compex-events_SVR.txt"); 
	...
	public void update(EventBean[] arg0, EventBean[] arg1) {
		...
		for (EventBean event : arg0) {
			detectedComplexEvent = event.get("timestamp") 
					...
					+ "," + event.get("temperatureWarnings")
					+ "," + event.get("temperatureMax")
